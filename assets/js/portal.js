...
const createHogaManagedInitialAccess = httpsCallable(functions, 'createHogaManagedInitialAccess');
const sendHogaManagedInitialAccessMail = httpsCallable(functions, 'sendHogaManagedInitialAccessMail');
...
async function loadCustomerAccesses(id){
  ...
  try{const result=await listHogaManagedInitialAccesses({customerId:id}),rows=result.data.accesses||[];const labels={not_configured:'Noch nicht eingerichtet',configured:'Eingerichtet – Einladung ausstehend',undocumented:'Einladungsstatus nicht dokumentiert',invited:'Einladung versendet – Aktivierung ausstehend',activated:'Aktiviert'};const date=value=>value?new Intl.DateTimeFormat('de-DE',{dateStyle:'medium',timeStyle:'short'}).format(new Date(value)):'–';target.innerHTML=`<h3>Zugänge</h3><p class="muted-text">Jede Aktion gilt ausschließlich für den hier aufgeführten Anwendungszugang. Kundeneigene Benutzer werden nicht geladen.</p><table class="portal-table"><thead><tr><th>Anwendung</th><th>E-Mail</th><th>Status</th><th>Letzte Versände</th><th>Aktionen</th></tr></thead><tbody>${rows.map(a=>`<tr><td>${escapeHtml(a.application)}</td><td>${escapeHtml(a.email||'–')}</td><td><strong>${escapeHtml(labels[a.status]||a.status)}</strong>${a.activated===true?'<span class="admin-subline">Durch Anmeldung bestätigt</span>':'<span class="admin-subline">Passwort noch nicht durch Anmeldung bestätigt</span>'}</td><td><span class="admin-subline">Einladung: ${date(a.invitationSentAt)}</span><span class="admin-subline">Startlink: ${date(a.startLinkSentAt)}</span><span class="admin-subline">Passwort-Reset: ${date(a.passwordResetSentAt)}</span></td><td class="table-actions">${a.status==='not_configured'?`<button class="table-action" type="button" data-create-initial-access="${escapeHtml(a.applicationId)}">Erstzugang einrichten</button>`:a.activated===true?`<button class="table-action secondary" type="button" data-access-action="start" data-access-application="${escapeHtml(a.applicationId)}">Startlink senden</button><button class="table-action secondary" type="button" data-access-action="passwordReset" data-access-application="${escapeHtml(a.applicationId)}">Passwort zurücksetzen</button>`:`<button class="table-action" type="button" data-access-action="invitation" data-access-application="${escapeHtml(a.applicationId)}">Einladungslink senden</button>`}</td></tr>`).join('')}</tbody></table>`;target.querySelectorAll('[data-create-initial-access]').forEach(button=>button.addEventListener('click',()=>createInitialAccess(id,button.dataset.createInitialAccess)));target.querySelectorAll('[data-access-action]').forEach(button=>button.addEventListener('click',()=>sendManagedAccessAction(id,button.dataset.accessApplication,button.dataset.accessAction)));}catch(e){console.error(e);target.innerHTML='<div class="empty-state">Zugangsstatus konnte nicht geladen werden.</div>';}
}
async function createInitialAccess(customerId,application){const customer=adminData.customers.find(c=>c.id===customerId);if(!customer)return;if(application==='customerPortal'&&!customer.email){showPortalMessage('Bitte zuerst eine E-Mail-Adresse beim Kunden hinterlegen.','error');return;}try{await createHogaManagedInitialAccess({customerId,application,email:customer.email||'',displayName:customer.contactName||customer.name});await loadCustomerAccesses(customerId);showPortalMessage('Der Erstzugang wurde eingerichtet. Es wurde noch keine E-Mail versendet.');}catch(error){console.error(error);showPortalMessage(error?.message||'Der Erstzugang konnte nicht eingerichtet werden.','error');}}
async function sendManagedAccessAction(customerId,application,action){const actionLabels={invitation:'Einladungslink',start:'Startlink',passwordReset:'Passwort-Reset-Link'};try{const result=await sendHogaManagedInitialAccessMail({customerId,application,action});await loadCustomerAccesses(customerId);const suffix=action==='start'?' Passwort und Status blieben unverändert.':action==='passwordReset'?' Das bisherige Passwort bleibt gültig, bis der Kunde den Reset abschließt.':'';showPortalMessage(`${actionLabels[action]} für ${result.data.application} wurde an ${result.data.email} versendet.${suffix}`);}catch(error){console.error(error);showPortalMessage(error?.message||`${actionLabels[action]||'Der Link'} konnte nicht versendet werden.`,'error');}}
