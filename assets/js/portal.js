...
async function loadCustomerAccesses(id){
  ...
  try{const result=await listHogaManagedInitialAccesses({customerId:id}),rows=result.data.accesses||[];const labels={not_configured:'Noch nicht eingerichtet',configured:'Eingerichtet, Link noch nicht versendet',undocumented:'Versandstatus nicht dokumentiert',sent:'Zugangslink versendet'};target.innerHTML=`<h3>Zugänge</h3><p class="muted-text">Nur von HOGAsports verwaltete Erstzugänge. Kundeneigene Benutzer werden hier nicht geladen.</p><table class="portal-table"><thead><tr><th>Anwendung</th><th>Erstzugang</th><th>Status</th><th>Letzter Linkversand</th><th>Aktivierung</th><th>Aktion</th></tr></thead><tbody>${rows.map(a=>`<tr><td>${escapeHtml(a.application)}</td><td>${escapeHtml(a.email||'–')}</td><td>${escapeHtml(labels[a.status]||a.status)}</td><td>${a.lastSentAt?new Intl.DateTimeFormat('de-DE',{dateStyle:'medium',timeStyle:'short'}).format(new Date(a.lastSentAt)):'–'}</td><td>${a.activated===true?'Durch Anmeldung bestätigt':'Nicht verlässlich feststellbar'}</td><td>${a.status==='not_configured'?`<button class="table-action" type="button" data-create-initial-access="${escapeHtml(a.applicationId)}">Erstzugang einrichten</button>`:`<button class="table-action secondary" type="button" data-send-initial-access="${escapeHtml(a.applicationId)}">${a.activated===true?'Passwort zurücksetzen':'Zugangslink senden'}</button>`}</td></tr>`).join('')}</tbody></table>`;target.querySelectorAll('[data-create-initial-access]').forEach(button=>button.addEventListener('click',()=>createInitialAccess(id,button.dataset.createInitialAccess)));target.querySelectorAll('[data-send-initial-access]').forEach(button=>button.addEventListener('click',()=>sendInitialAccess(id,button.dataset.sendInitialAccess)));}catch(e){console.error(e);target.innerHTML='<div class="empty-state">Zugangsstatus konnte nicht geladen werden.</div>';}
}
...
async function sendInitialAccess(customerId,application){try{const result=await sendHogaManagedInitialAccessMail({customerId,application});await loadCustomerAccesses(customerId);showPortalMessage(`Der sichere Zugangslink wurde an ${result.data.email} versendet. Ein bestehendes Passwort wird erst geändert, wenn der Empfänger den Link verwendet.`);}catch(error){console.error(error);showPortalMessage(error?.message||'Der Zugangslink konnte nicht versendet werden.','error');}}
